...
/// subject: 邮件主题
/// content: 邮件内容
pub fn send_with_file(receiver: String, subject: String, content: String, files: Vec<String>) -> anyhow::Result<()> {
    // get AUTHORIZE CODE from env param
    let authorize_code =
        std::env::var("EMAIL_AUTHORIZE_CODE").expect("Cannot get env EMAIL_AUTHORIZE_CODE");
    // 发件人邮箱
        ...
        multipart = multipart.clone().singlepart(
            SinglePart::builder()
                .header(header::ContentType::parse("image/jpeg").expect("解析图片格式失败:"))
                        ...
                        // 注意demo.jpg用尖括号包裹起来了
                        .header(header::ContentId::parse(&format!("<{file}>")).expect("解析成content-id失败:"))
